...
def _load_run(output_dir: Path, n_epochs: int, seed: int) -> dict | None:
    # Table 8's study is explicitly nominal-only (paper: "for the nominal
    # VAE") -- train_vae.py now prefixes output filenames with scenario, so
    # this is made explicit rather than relying on the old, ambiguous
    # (pre-scenario-flag) naming convention.
    path = output_dir / f"nominal_n_ep_{n_epochs}_seed_{seed}.json"
    if not path.exists():
        return None
